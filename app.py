...
st.set_page_config(
    page_title="QuLab: Lab 56: On-chain Anomaly Detection", layout="wide")
st.sidebar.image("https://www.quantuniversity.com/assets/img/logo5.jpg")
st.sidebar.divider()
st.title("QuLab: Lab 56: On-chain Anomaly Detection")
st.divider()
...
if 'txns_with_alerts' not in st.session_state:
...
# Determine current index for selectbox to maintain state
try:
...
# Main application title logic is handled above, specific page content follows
...
if st.session_state.page == "Overview":
    ...
    st.markdown(
        f"1.  **Simulate Data**: Generate synthetic blockchain transactions with injected anomalies.")
    st.markdown(
        f"2.  **Engineer Features**: Create crypto-specific features from raw transaction data.")
    st.markdown(
        f"3.  **Detect Anomalies**: Implement a two-layer anomaly detection system (Z-score + Isolation Forest).")
    st.markdown(
        f"4.  **Classify Anomalies**: Categorize detected anomalies into actionable types.")
    st.markdown(
        f"5.  **Market Impact Analysis**: Assess the market's reaction to different anomaly types.")
    st.markdown(
        f"6.  **Monitoring Dashboard**: Define an alert protocol for operationalizing risk management.")

elif st.session_state.page == "1. Simulate Data":
    st.markdown(
        f"## 1. Simulating the Blockchain: Crafting Our Testbed of Risks")
    ...
    if st.button("Generate Blockchain Data"):
        ...
        st.markdown(
            f"**Total transactions simulated**: {len(st.session_state.txns)}")
        st.markdown(
            f"**Number of anomalies injected**: {(st.session_state.txns['anomaly_type'] != 'normal').sum()}")
        for atype in ('whale', 'hack', 'wash'):
            st.markdown(
                f"  - {atype.capitalize()} movements: {(st.session_state.txns['anomaly_type'] == atype).sum()}")
        st.markdown(f"")
        st.markdown(f"**First 5 transactions:**")
        st.dataframe(st.session_state.txns.head())
    else:
        if st.session_state.txns is not None:
            st.info("Data already generated. Click button to regenerate.")
            st.dataframe(st.session_state.txns.head())
        else:
            st.info("Click 'Generate Blockchain Data' to begin the simulation.")

elif st.session_state.page == "2. Engineer Features":
    st.markdown(
        f"## 2. Engineering On-Chain Features: Unlocking Deeper Insights")
...
""")
    ...
    if st.session_state.txns is None:
        st.warning(
            "Please simulate blockchain data first from the '1. Simulate Data' page.")
    else:
        if st.button("Engineer On-Chain Features"):
            with st.spinner("Engineering features..."):
                st.session_state.txns_engineered = engineer_onchain_features(
            ...
            st.markdown(
                f"**On-chain features engineered**: {len(st.session_state.feature_cols)}")
            st.markdown(f"**First 5 transactions with new features:**")
            st.dataframe(
                st.session_state.txns_engineered[st.session_state.feature_cols + ['anomaly_type']].head())
            ...
            sns.histplot(st.session_state.txns_engineered['amount_log'], bins=50,
                         kde=True, label='All Transactions', color='gray', ax=ax)
            ...
            ax.set_title(
                'Log-transformed Transaction Amount Distribution with Anomalies')
            ...
            st.pyplot(fig)
            plt.close(fig)
        elif st.session_state.txns_engineered is not None:
            st.info("Features already engineered. Click button to re-engineer.")
            st.dataframe(
                st.session_state.txns_engineered[st.session_state.feature_cols + ['anomaly_type']].head())

elif st.session_state.page == "3. Detect Anomalies":
...
""")
    ...
    if st.session_state.txns_engineered is None:
        st.warning(
            "Please engineer features first from the '2. Engineer Features' page.")
    else:
        if st.button("Run Two-Layer Anomaly Detection"):
            with st.spinner("Detecting anomalies..."):
                ...
                zscore_results = zscore_alerts(
                    df_temp, amount_threshold=4.0, dormancy_threshold=3.0)
                df_temp['any_zscore_alert'] = zscore_results['any_zscore_alert']

                # Layer 2: Isolation Forest
                scaler = StandardScaler()  # Instantiate a new scaler
                X = scaler.fit_transform(
                    df_temp[st.session_state.feature_cols].fillna(0))
                iso_forest = IsolationForest(
                    # Instantiate a new model
                    n_estimators=200, contamination=0.005, random_state=42, n_jobs=-1)
                iso_forest.fit(X)
                df_temp['iso_forest_anomaly_score'] = iso_forest.decision_function(
                    X)
                df_temp['is_flagged_iso_forest'] = (
                    iso_forest.predict(X) == -1).astype(int)

                # Combine layers for final alert
                df_temp['final_alert'] = (
                    df_temp['is_flagged_iso_forest'] == 1) | df_temp['any_zscore_alert']

                st.session_state.txns_with_alerts = df_temp

                # Evaluate overall detection performance
                true_anomalies = (
                    st.session_state.txns_with_alerts['anomaly_type'] != 'normal')
            ...
            st.markdown(f"### Explanation of Execution")
            st.markdown(
                f"The two-layer anomaly detection system successfully flagged a significant number of suspicious transactions. The overall recall of {st.session_state.overall_recall:.2%} indicates that we are effectively capturing most of the true anomalous events, which is critical for risk management. The precision of {st.session_state.overall_precision:.2%} suggests that while we catch many true anomalies, there are also some false positives. This is acceptable in a risk management context, where missing a critical event (false negative) is more costly than investigating a benign one (false positive).")
            st.markdown(
                f"**Transactions flagged by Z-score alerts**: {(df_temp['any_zscore_alert']).sum()}")
            st.markdown(
                f"**Transactions flagged by Isolation Forest**: {(df_temp['is_flagged_iso_forest']).sum()}")
            st.markdown(
                f"**Total transactions flagged by two-layer detection**: {(df_temp['final_alert']).sum()}")

            st.markdown(f"\nOVERALL DETECTION PERFORMANCE")
            st.markdown(f"True anomalies: {true_anomalies.sum()}")
            st.markdown(
                f"Flagged: {flagged.sum()} (TP={tp}, FP={fp}, FN={fn})")
            st.markdown(
                f"Recall: {st.session_state.overall_recall:.2%} | Precision: {st.session_state.overall_precision:.2%}")
            ...
            ax.set_title(
                'Transaction Timeline with Injected and Detected Anomalies')
            ...
            st.pyplot(fig)
            plt.close(fig)
        elif st.session_state.txns_with_alerts is not None:
            st.info("Anomaly detection already run. Click button to re-run.")

elif st.session_state.page == "4. Classify Anomalies":
    st.markdown(
        f"## 4. Classifying Anomalies: Translating Signals into Actionable Intelligence")
    ...
    if st.session_state.txns_with_alerts is None:
        st.warning(
            "Please run anomaly detection first from the '3. Detect Anomalies' page.")
    else:
        if st.button("Classify Detected Anomalies"):
            with st.spinner("Classifying anomalies..."):
                ...
                flagged_txns['classified_type'] = flagged_txns.apply(
                ...
                for atype_injected in ('whale', 'hack', 'wash'):
                    ...
                    total_injected_type = (
                        df_temp['anomaly_type'] == atype_injected).sum()
                    recall_type = tp_classified_correctly / \
                        max(total_injected_type, 1)
                    st.markdown(
        ...
        elif st.session_state.flagged_txns is not None:
            st.info("Anomalies already classified. Click button to re-classify.")

elif st.session_state.page == "5. Market Impact Analysis":
    st.markdown(
        f"## 5. Assessing Market Impact: From On-Chain Signal to Portfolio Action")
    ...
    if st.session_state.flagged_txns is None:
        st.warning(
            "Please classify anomalies first from the '4. Classify Anomalies' page.")
    else:
        if st.button("Run Market Impact Analysis"):
            with st.spinner("Analyzing market impact..."):
                ...
                # Call for potential side-effects like printing to console (not used directly in Streamlit display)
                market_impact_analysis(st.session_state.flagged_txns)
                ...
                st.markdown(
                    f"{'Event Type':<25s} {'1h Impact':>10s} {'4h Impact':>10s} {'24h Impact':>10s}")
                st.markdown(f"-" * 60)
                for etype, impact in impacts.items():
                    st.markdown(
                        f"{etype:<25s} {impact['1h']:>+10.1f}% {impact['4h']:>+10.1f}% {impact['24h']:>+10.1f}%")

                st.markdown(f"\n**Key findings:**")
                st.markdown(
                    f" - Whale movements precede an average -2.1% price drop within 4h.")
                st.markdown(
                    f" - Hack events precede an average -5.2% price drop within 4h, indicating severe market reaction.")
                st.markdown(
                    f" - Wash trading has minimal directional impact (noise, not a strong signal).")
                ...
                melted_impact_df = impact_df.melt(
                    id_vars='Anomaly Type', var_name='Time Window', value_name='Avg Price Change (%)')

                fig, ax = plt.subplots(figsize=(12, 7))
                sns.barplot(x='Anomaly Type', y='Avg Price Change (%)',
                            hue='Time Window', data=melted_impact_df, palette='viridis', ax=ax)
                ax.set_title(
                    'Average BTC Price Change Following Different Anomaly Types')
...
elif st.session_state.page == "6. Monitoring Dashboard":
    st.markdown(
        f"## 6. Building a Real-Time Alert System: Operationalizing Risk Management")
    ...
    if st.button("Display Monitoring Dashboard Protocol"):
        with st.spinner("Compiling protocol..."):
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    f"**Latency**: < 30 seconds from block confirmation to alert")
                st.markdown(
                    f"**Data sources**: Block explorer API (Etherscan, Blockchain.com)")
                st.markdown(
                    f"**Monitoring agent**: AI agent from D3-T3 running continuously")
            st.markdown(f"### Explanation of Execution")
